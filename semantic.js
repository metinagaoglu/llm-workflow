...
const callbackHandler = new CallbackHandler({
    secretKey: "sk-lf-61d80c4b-a582-4293-9d0d-37dab4421c85",
    publicKey: "pk-lf-00fac1f2-ef8d-44ec-9679-4e7c896d920e",
...
async function orchestrator(state) {
    console.log("Orchestrator State:", state); // Debug için
    const userId = state.userId || state.config?.configurable?.user_id;
    ...
    const llmInput =
        ...
        Previous action: ${JSON.stringify(formattedHistory)}
...
async function handleLabelSearch(input) {
    console.log('handleLabelSearch');
    return "Bu numara için etiketler: 'Spam', 'Dolandırıcı'";
}
...
async function handleAddSpam(input) {
    console.log('handleAddSpam');
    return "Numara zaten spam olarak işaretlendi.";
}
...
async function synthesizer(state) {
    ...
    const result = await llm.invoke([
        {
            role: "system",
            content: "You are a response synthesizer. Combine multiple task results into a coherent, unified response. If there are multiple actions requested, explain each result clearly. Append the is there any other action to be done nicely. Use this countrys language: ID"
        },
        {
            role: "user",
...
async function runTest() {
    ...
    try {
        ...
        while (true) {
            ...
            let state = await workflow.invoke(initialState, config);
